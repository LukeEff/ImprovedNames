...
public class ClearSuffix implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (strings.length == 1) {
            ...
            ScoreboardCore.setNewPlayerSuffix(groupName, ""); //TODO Just grab it from the database
            ScoreboardCore.updateScoreBoard();
            ((Player) commandSender).sendMessage(ChatColor.GREEN + "Success. Removed suffix from player " + groupName);
        }
        return true;
    }
}
